# IGA_module_2.py の print メッセージは「シアン」で表示される
import random
from PIL import Image
from GENE import Gene
from GPT import mutate

def interactive_Genetic_algorithm(genes, this_images):
    print("\033[96m選択された遺伝的アルゴリズムは2番。基本的な方針による選択、交叉、突変を行います。\033[0m")
    # 次の遺伝子となる8個のgene型のリストを宣言
    new_genes = []
    for i in range(8):
        new_genes.append(Gene(Image.new("RGB", (256, 256)), 0.0, 0, 0, 0, 0.0, [], 0, ""))
    ...
    for _ in range(8):
        ...
        parent_image_pairs.append((this_images[choiced_index[0]], this_images[choiced_index[1]]))
        print(f"\033[96m選択された両親の遺伝子のインデックス: {choiced_index}\033[0m")

    # 交叉
    # [image 交叉]
    # 両親の gene.init_image 又は、両親の this_image という計4枚の画像から1枚の画像を選択していく
    for i in range(8):
        # 両親の画像をランダムに選択
        images = [
            parent_gene_pairs[i][0].init_image,  # 1
            parent_gene_pairs[i][1].init_image,  # 2
            parent_image_pairs[i][0],            # 3
            parent_image_pairs[i][1]             # 4
        ]
        # 画像をランダムに選択
        choiced_image = random.choice(images)
        new_genes[i].init_image = choiced_image

        # 選択された画像を識別するためのインデックスを取得
        choiced_image_index = images.index(choiced_image) + 1

        # 選択された画像を識別するためのメッセージを表示
        if choiced_image_index == 1:
            print(f"\033[96m選択された画像: 1 (parent_gene_pairs[{i}][0].init_image)\033[0m")
        elif choiced_image_index == 2:
            print(f"\033[96m選択された画像: 2 (parent_gene_pairs[{i}][1].init_image)\033[0m")
        elif choiced_image_index == 3:
            print(f"\033[96m選択された画像: 3 (parent_image_pairs[{i}][0])\033[0m")
        elif choiced_image_index == 4:
            print(f"\033[96m選択された画像: 4 (parent_image_pairs[{i}][1])\033[0m")
    ...
    mutate_image_list = []
    for i in range(8):
        mutate_image_list.append(parent_gene_pairs[i][0].init_image)
        mutate_image_list.append(parent_gene_pairs[i][1].init_image)
        mutate_image_list.append(parent_image_pairs[i][0])
        mutate_image_list.append(parent_image_pairs[i][1])

    for i in range(8):
        if random.random() < 0.1:
            print(f"\033[96mgene[{i}]のinit_imageを{new_genes[i].init_image}から突然変異\033[0m")
            new_genes[i].init_image = random.choice(mutate_image_list)
            print(f"\033[96mnew_genes[{i}].init_image: {new_genes[i].init_image}\033[0m")
        
    # image_strengs 突変は、0.1の確率で0.1から0.4の間でランダムに生成
    for i in range(8):
        if random.random() < 0.1:
            print(f"\033[96mgene[{i}]のimage_strengsを{new_genes[i].image_strengs}から突然変異\033[0m")
            new_genes[i].image_strengs = round(random.uniform(0.1, 0.4), 2)
            print(f"\033[96mnew_genes[{i}].image_strengs: {new_genes[i].image_strengs}\033[0m")
    ...
    for i in range(8):
        if random.random() < 0.1:
            print(f"\033[96mgene[{i}]のpromptを{new_genes[i].prompt}から突然変異\033[0m")
