...
class Gene:
    def __init__(self, init_image: Image.Image, image_strengs: float, seed: int, steps: int, prompt_length: int, cfg_scale: float, prompt: List[str]):
...
def create_base_genes(input_text, input_image):
    ...
    for i in range(8):
        ...
        gene = Gene(input_image, image_strings, seed, steps, prompt_length, cfg_scale, prompt_list[i])
        genes.append(gene)
